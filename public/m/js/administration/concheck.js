...
function checkStart(cid) {
	$('.check-wrap').addClass('showed')
	$('.check-wrap').fadeIn(200)
	ap.list.hide()
	getConInfos(cid)
}
$(document).on('click', '.check-wrap.showed .local-header .btn-back.back', function () {
	$('.check-wrap').removeClass('showed')
	$('.check-wrap').fadeOut(100)
})
