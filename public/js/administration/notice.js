...
function getContent(type){
	...
	if(type == "publish"){
		$(".notices-content .title").html("发布公告")
		$.get("/user/infos", function (data, err) {
			if (data.code !== 0) {
				if (data.code == -502) {
					alert('请先登录')
			...
			var content = ""
			content = "<div class='publish-wrap'><div class='edit-wrap'><div class='input-rows'><div class='input-label'>公告标题:</div><div class='input-wrap large title-wrap'><span class='input-span'><input type='text' class='editable' id='title' autocomplete='on'placeholder='公告标题' /></span><span class='clear-span title-clear'>×</span></div><p class='message' id='title'></p></div><div class='input-rows'><div class='input-label'>发布署名:</div><div class='input-wrap large auther-wrap'><span class='input-span'><input type='text' class='editable' id='auther' autocomplete='on'placeholder='发布署名' /></span><span class='clear-span auther -clear'>×</span></div><p class='message' id='auther'></p></div><div class='input-rows'><div class='input-label'>发布时间:</div><div class='input-wrap large time-wrap'><span class='input-span'><input type='text' class='editable' id='time' autocomplete='on'placeholder='发布时间' /></span><span class='clear-span time-clear'>×</span></div><p class='message' id='time'></p><div class='get-now-time' title='填入当前时间'><span class='get-now-time-span'><i class='fa fa-clock-o fa-lg'></i></span></div></div><div class='input-rows'><div class='input-label'>发布对象:</div><div class='input-wrap large to-user-wrap'><span class='input-span'><input type='text' class='readonly' id='to-user' autocomplete='on'placeholder='发布对象' readonly='readonly' value='全体（暂不支持修改）'/></span><!--<span class='clear-span to-user-clear'>×</span>--></div><p class='message' id='to-user'></p></div><div class='textarea-wrap'><textarea id='text-md' rows='15' cols='100' placeholder='公告内容（markdown语法格式）'></textarea></div></div><div class='btn-preview'><div class='input-wrap-btn large'><input type='button' class='btn btn-get' value='预览' id='preview' onclick='getPreview()'></div></div><div class='preview-wrap'></div></div>"
			return $(".notices-content .list").html(content)
		})
	}
	if(type == "archive"){
		$(".notices-content .title").html("公告管理")
...
function getPreview(){
	...
	"</div>" + auther_text + "</div>"
	notice_content += "<div class='btn-publish'><div class='input-wrap-btn large'><input type='button' class='btn btn-get' value='发布' id='publish' onclick='Publish()'></div></div>"
