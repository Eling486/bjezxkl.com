		...
		if(hash){
			...
			if(hash !== ""){
				...
				$('.comment-list').animate({
					scrollTop: $("#com-" + com_id + "").offset().top - $('.comment-list').offset().top + $('.comment-list').scrollTop() - 10
				},500,function(){
...
//取消回复
$('.com-reply-wrap').on('click', '.cancel-reply', function () {
    cancelReply()
})
...
function addReply(infos) {
    ...
    replyTips += "<span class='reply-to-comment-infos' style='display: none;'>" + JSON.stringify(infos) +
        "</span><span class='reply-to-text'> 回复 " + infos.user + " ： <span class='com-reply-cancel'><a href='javascript:;' class='cancel-reply'>取消回复</a></span></span>"
...
$('.send-button-wrap').on('click', '.send-btn.active', function () {
    var com_content = $('.comment-textarea').val()
    var music_infos = JSON.parse($('.music-infos .musicinfos-text .musicinfos#infos').html())
    ...
    if (com_content !== "") {
		$('.send-button-wrap .send-btn.active').removeClass('active')
    ...
    $.ajax({
        ...
        success: function (data, status) {
            if (data.code !== 0) {
                if (data.code == -502) {
                    alert('请先登录')
					$('.send-button-wrap .send-btn').addClass('active')
                    showQuickLogin()
                }
                return console.log(data)
            }
...
function loginSucceeded() {
	if($('.music-infos .musicinfos-text .musicinfos#infos').html() !== ""){
		var music_infos = JSON.parse($('.music-infos .musicinfos-text .musicinfos#infos').html())
		getCommentsByMid(music_infos.mid)		
	}
}